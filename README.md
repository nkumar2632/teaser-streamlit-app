...
**Fetch ESPN NFL** and **Fetch ESPN CFB** use the same review and confirmation gate, targeting the next NFL Sunday or CFB Saturday. The URL field remains available for other game days and supported dated scoreboards. **Compare reference vs my sportsbook** lets you select stored snapshots for the chosen league. It defaults to the newest execution snapshot and a reference snapshot from the same season/week when available. Matched games show both sides' spread, spread price and moneyline, plus total and literal point differences (execution minus reference). Capture times, stale warnings, missing values, and unmatched or ambiguous games remain visible. This read-only view makes no betting recommendation and never substitutes reference data for actual sportsbook input.
